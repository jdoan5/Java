...
public class Main {
    ...
    public static void main(String[] args) {
        // Stage 2: repository backed by CSV on disk (persists across runs)
        Path csvPath = Paths.get("CSV", "job_applications.csv");
        ApplicationRepository repository = new ApplicationRepository(csvPath);
        ApplicationService service = new ApplicationService(repository);

        int loaded = repository.findAll().size();
        System.out.println("=== Job Application Tracker (Java, Console, Stage 2) ===");
        if (loaded > 0) {
            System.out.println("Loaded " + loaded + " application(s) from " + csvPath + ".");
        } else {
            System.out.println("No existing applications found. Starting fresh.");
        }
