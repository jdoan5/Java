...
public class TicketController {
    ...
    // NOTE: no @Valid here, deliberately. PATCH is a PARTIAL update, so callers send only the
    // fields they want changed (the Angular client sends just {"status": ...}). UpdateTicketRequest
    // is shared with PUT and its @NotBlank/@NotNull constraints would reject every partial body.
    // See TicketControllerTest#patchWithOnlyAStatusStillSucceeds, which guards this.
    @PatchMapping("/{id}")
