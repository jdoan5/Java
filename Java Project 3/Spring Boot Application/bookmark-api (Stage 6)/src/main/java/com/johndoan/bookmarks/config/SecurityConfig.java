...
public class SecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http
                .authorizeHttpRequests(auth -> auth
                        // "/actuator/health/**" — not just "/actuator/health" — is what makes
                        // the Kubernetes probes work. kubelet calls /actuator/health/liveness
                        // and /actuator/health/readiness with no credentials, and an
                        // exact-path rule answers those 401, which Kubernetes reads as
                        // "unhealthy" and restart-loops the pod.
                        // "/" is the public landing page. It must be permitAll, otherwise a
                        // signed-out visitor is bounced to /login, and after logging in
